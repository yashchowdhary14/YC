...
import { Grid3x3, Clapperboard, AtSign } from 'lucide-react';
...
export default function Profile2Page() {
  ...
  // For this standalone Profile 2.0, we'll feature a specific user from dummy data.
  const featuredUsername = 'sachin';
  ...
  useEffect(() => {
    setIsLoading(true);
    // Fetch data for the featured user from dummy data.
    const user = dummyUsers.find(u => u.username.toLowerCase() === featuredUsername.toLowerCase());
  ...
  const profileUser: ProfileUser | null = useMemo(() => {
    ...
    // In this case, we compare the featured user's ID with the current user's ID
    const isCurrentUser = currentUser.uid === profileData.user.id;
    const isFollowing = !isCurrentUser && followedUsers.has(profileData.user.username);
  ...
  if (!profileUser) {
    // This could happen if the featured user isn't found in dummy data
    return notFound();
  }

  const reels = profileData.posts.filter(p => p.type === 'reel' || p.type === 'video');
  const taggedPosts = dummyPosts.slice(5, 11).reverse(); // just some random posts for demo

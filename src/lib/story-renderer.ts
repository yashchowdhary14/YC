...
import type { MediaObject, TextElement, DrawingElement } from './create-store';
import { getSvgPathFromPoints, drawText, getFitDimensions, loadImage, loadVideo, generateThumbnailFromCanvas } from './story/render-utils';
import { filters as filterPresets } from '@/components/story-creation/FilterStrip';
import { interpolateFilter } from './story/filterUtils';
import { renderEffects } from './story/renderEffects';
...
async function renderImageStory(state: MediaObject): Promise<RenderedStoryOutput> {
...
async function renderVideoStory(state: MediaObject): Promise<RenderedStoryOutput> {
    const video = await loadVideo(state.url);
...
function renderDrawing(ctx: CanvasRenderingContext2D | OffscreenCanvasRenderingContext2D, drawings: MediaObject['drawings']) {
